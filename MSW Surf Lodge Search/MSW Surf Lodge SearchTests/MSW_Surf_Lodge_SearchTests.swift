...
class MSW_Surf_Lodge_SearchTests: XCTestCase {
    ...
    func testNearbyEndpointURL() {
        // This is assuming the API Key is empty. If the API Key is present, please modify the url it should equal or temporarily remove the API key.
        let nearbyRequestURL = LodgeURL.nearbySearch(location: CLLocationCoordinate2D(latitude: 50.41563, longitude: -5.07521))
    ...
    func testNearbyAPICall(){
        //this test needs to be conducted with a valid network connection with an API key installed (Install the API Key in Endpoint.swift // Line 14)
        let surfLodgeAPI = SurfLodgeAPI()
        //call the api
        surfLodgeAPI.getLodges(location: CLLocationCoordinate2D(latitude: 50.41563, longitude: -5.07521)) { (results, error) in
            XCTAssertTrue(error == nil)
        }
    }
    
    func testDetailEndpointURL(){
        // This test case assumes the API Key is empty. If the API Key is present, please modify the url it should equal or temporarily remove the API key.
    ...
    func testDetailAPICall(){
        ...
        let surfLodgeAPI = SurfLodgeAPI()
        //call the API
        surfLodgeAPI.getDetails(placeId: "ChIJy7adMN7dakgR47sILhSc5QM") { (details, error) in
            XCTAssertTrue(error == nil)
        }
    }
